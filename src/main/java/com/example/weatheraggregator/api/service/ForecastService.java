...
import static com.example.weatheraggregator.api.service.ForecastUtil.*;
...
public class ForecastService {
    ...
    public ForecastPageDTO getForecastsAggregated(ForecastRequest request, PageRequest pageRequest) {
        ...
        Page<Forecast> forecasts = convertMeasurementUnits(forecastRepository.findForCity(city, type,
                request.getStartDate().atStartOfDay(), request.getEndDate().atStartOfDay(), true,
                        pageRequest), request.getUnit());

        return new ForecastPageDTO(ForecastUtil.aggregateForecasts(forecasts), type.getType(), new CityDTO(city),
                "Aggregated");
    }
}
